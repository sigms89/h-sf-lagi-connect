...
export function useBenchmarkData(
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<BenchmarkRow[]> => {
      ...
      // Filter by postal prefix and building year client-side
      const filteredAssocs = (otherAssocs ?? []).filter((a: any) => {
        if (filters.postalPrefix !== 'all') {
          if (!(a.postal_code ?? '').startsWith(filters.postalPrefix)) return false;
        }
        if (a.building_year != null) {
          if (a.building_year < filters.buildingYearFrom || a.building_year > filters.buildingYearTo) {
            return false;
          }
        }
        return true;
